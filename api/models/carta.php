...
class Carta
{
	...
	public function readCartaActivaByIdEstabliment()
	{
		$query = "SELECT carta.nom, carta.idCarta, carta.actiu from carta, carta_establiment where carta.idCarta = carta_establiment.Carta_idCarta 
		and carta_establiment.Establiment_id = ? AND actiu != 0";
		$stmt = $this->conn->prepare($query);
	...
	{
		$query = "SELECT carta.nom, carta.idCarta, carta.actiu from carta, carta_establiment where carta.idCarta = carta_establiment.Carta_idCarta 
		and carta_establiment.Establiment_id = ?;";
		$stmt = $this->conn->prepare($query);
		$stmt->bind_param('i', $this->idEstabliment);
		$stmt->execute();
		return $stmt->get_result();
